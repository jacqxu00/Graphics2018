...
def scanline_convert(polygons, i, screen, zbuffer ):
    ...
    # create an array in order of y coordinate
    ...
    # draw colored lines
    color = [i*597120%255+60, i*457950%255+50, i*123630%255+70]
    x_bt = xb
    x_bmt = xb
    z_bt = zb
    z_bmt = zb
    ...
    while y < int(ym):
        y += 1
        draw_line(int(x_bt), int(y), int(z_bt), int(x_bmt), int(y), int(z_bmt), screen, zbuffer, color)
        x_bt += (xt - xb) / (yt - yb)
        z_bt += (zt - zb) / (yt - yb)
        x_bmt += (xm - xb) / (ym - yb)
        z_bmt += (zm - zb) / (ym - yb)
